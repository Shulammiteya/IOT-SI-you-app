...
public class PieChartItem extends ChartItem {
    ...
    public PieChartItem(ChartData<?> cd, Context c, String name) {
        ...
        mCenterText = generateCenterText(name);
    }
    ...
    public View getView(int position, View convertView, Context c) {
        ...
        holder.chart.setExtraOffsets(20f, 50.f, 30.f, 50.f);
    ...
    private SpannableString generateCenterText(String name) {
        SpannableString s = new SpannableString("SI you\n\n資料屬於\n" + name);
        s.setSpan(new RelativeSizeSpan(2f), 0, 6, 0);
        s.setSpan(new ForegroundColorSpan(ColorTemplate.VORDIPLOM_COLORS[0]), 0, 6, 0);
        s.setSpan(new RelativeSizeSpan(.9f), 6, 12, 0);
        s.setSpan(new ForegroundColorSpan(Color.GRAY), 6, 12, 0);
        s.setSpan(new RelativeSizeSpan(1.2f), 12, s.length(), 0);
